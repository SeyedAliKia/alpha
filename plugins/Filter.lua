...
local function run(msg, matches)
  if matches[1]:lower() == 'filter' or matches[1] == 'فیلتر' and is_momod(msg) then
    local name = string.sub(matches[2], 1, 50)
    local text = addword(msg, name)
    return text
  end

  if matches[1] == 'filterlist' or matches[1] == 'لیست فیلتر' and is_momod(msg) then
    return list_variablesbad(msg)
  end

  if matches[1] == 'clean' or matches[1] == 'حذف' and matches[2] == 'filterlist' or matches[2] == 'لیست فیلتر' and is_owner(msg) then
    local asd = '1'
    return clear_commandbad(msg, asd)
  end

  if matches[1] == 'unfilter' or matches[1] == 'حذف فیلتر' and is_momod(msg) then
